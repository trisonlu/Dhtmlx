...
function dhtmlXTabBar(parentObject, mode, height) {
    ...
    setSkin: function (name) {
        ...
        if (sky_mode)this._s.skin_line = 0;
        if (simple_mode) {
        ...
        if (sky_mode) {
            ...
            var f = function () {
                ...
                if (this._lastActive)w = Math.max(w, this._lastActive.parentNode[this._s._vMode ? "scrollHeight" : "scrollWidth"]);
                if (w < 6)return;
    ...
    enableAutoReSize: function () {
        ...
        dhtmlxEvent(window, "resize", function () {
            window.setTimeout(function () {
                if (self && self._setSizes)self._setSizes();
            }, 1)
        })
    },
    _createSelf: function () {
        ...
        if (this._s._vMode)this._tabAll.className += 'V';
        if (this._s._bMode)this._tabAll.className += 'B';
    ...
    _removeRow: function (row) {
        ...
        var z = [];
        for (var i = 0; i < this._rows.length; i++)if (this._rows[i] != row)z[z.length] = this._rows[i];
        this._rows = z;
    },
    ...
    _checkScroll: function () {
        if (this._s._vMode || !this._s.scrolls)return;
        for (var i = 0; i < this._rows.length; i++)if ((this._rows[i].scrollWidth - this._rows[i].offsetWidth) > 2)
    ...
    _setRowSizes: function () {
        for (var i = 0; i < this._rows.length; i++) {
            ...
            this._rows[i].style[this._py] = i * (this._s.line_height - (this._s.skin_line_x ? 4 : 2)) - ((_isIE && !window.postMessage && this._s._bMode) ? 0 : 0) + "px";
    ...
    addTab: function (id, text, size, position, row) {
        ...
        row = row || 0;
        for (var i = this._rows.length; i <= row; i++)this._createRow();
    ...
    _createTab: function (id, text, size) {
        ...
        tab.skin = this.skin;
        if (this._a.tab_color)tab.style.backgroundColor = this._a.tab_color;
        if (this._c[id])tab.childNodes[0].style.cssText = this._c[id].color;
        this._img_all(tab);
        if (this._close) {
            var self = this;
    ...
    _get_img_pos: function (ind) {
        ...
        ind = -5 - ind * 45;
        if (this._s._vMode)return ind + "px 0px"; else return "0px " + ind + "px";
    },
    ...
    _showHover: function (tab) {
        ...
        if (this._lastHower && this._lastHower != this._lastActive) {
            ...
            if (a.hover)this._img_all(this._lastHower); else
    ...
    _onClickHandler: function (e) {
        ...
        if (target && !target._disabled)this.tabbar._setTabActive(target); else {
            var tag = null;
            ...
            if (tag) {
                ...
                if (!_isIE || window.XMLHttpRequest)tag.nextSibling.style.right = tag.parentNode.scrollLeft * (-1) + "px";
                return false;
            }
        }
    },
    ...
    _setTabActive: function (tab, mode) {
        ...
        if (this._c[id])tab.childNodes[0].style.cssText = this._c[id].scolor;
        this._img_all(tab);
        this._setTabTop(tab);
    ...
    _scrollTo: function (tab) {
        ...
        if (tab.offsetLeft < tab.parentNode.scrollLeft || (tab.offsetLeft + tab.offsetWidth) > (tab.parentNode.scrollLeft + tab.parentNode.offsetWidth)) {
            ...
            if (tag && tag[0]) {
                ...
                if (!_isIE || window.XMLHttpRequest)tag[1].style.right = tag[1].parentNode.scrollLeft * (-1) + "px";
            }
        }
    },
    _setTabTop: function (tab) {
        var t = this._rows.length - 1;
        for (var i = 0; i < this._rows.length; i++)if (this._rows[i] == tab.parentNode) {
    ...
    cells: function (id) {
        ...
        if (!this._content[id]) {
            ...
            if (this.skin == "dhx_web" && d._setPadding)d._setPadding(1, "dhxcont_tabbar_dhx_web");
            this._content[id] = d;
            var self = this;
            d.show = function () {
            ...
            d.autoSize = function () {
                if (self._awdj || self._ahdj) {
                    if (!this.mainCont.offsetWidth)this.mainCont.style.width = Math.max(0, self.entBox.offsetWidth - 2) + "px";
                    if (self._ahdj)this.mainCont.style.height = "1px";
            ...
            d._activate = function () {
                if (this._delay)this._attachContent.apply(this, this._delay);
                this.activate();
                if (self._hrfmode != "iframe")this._delay = null;
            };
            d.activate = function () {
            ...
            d._doOnAttachURL = function () {
                if (self._awdj || self._ahdj)self.cells(id).autoSize();
    ...
    clearAll: function () {
        for (var id in this._tabs)this.removeTab(id, false);
        if (this._lineA) {
            var line = this._getCoverLine();
    ...
    _parseXML: function (that, a, b, c, obj) {
        ...
        if (acs)that._s.align = (acs == "right" || acs == "bottom");
        ...
        for (var i = 0; i < arows.length; i++) {
            ...
            for (var j = 0; j < atabs.length; j++) {
                ...
                if (that._hrfmode)that.setContentHref(id, atabs[j].getAttribute("href")); else
    ...
    hideTab: function (id, mode) {
        ...
        if (tab == this._lastActive && mode !== false)this.goToNextTab();
        this._setTabSizes(tab.parentNode);
    },
    ...
    normalize: function (limit, full) {
        ...
        var tabs = [];
        for (var j = 0; j < this._rows.length; j++)for (var i = 0; i < this._rows[j].tabCount; i++)tabs.push(this._rows[j].removeChild(this._rows[j].childNodes[0]));
        ...
        var last_tab = null;
        var i = 0;
        for (i; i < tabs.length; i++)if ((size + tabs[i]._size + this._a.margin) < limit) {
        ...
        for (var j = 0; j < this._rows.length; j++)this._setTabSizes(this._rows[j]);
        this._setSizes();
        if (this._lastActive = t)this._setTabTop(this._lastActive);
    },

    showInnerScroll: function () {
        for (var i in this._tabs)this.cells(i).dhxcont.mainCont.style.overflow = "auto";
    },


    getNumberOfTabs: function () {
        var tc = 0;
        for (var i = 0; i < this._rows.length; i++)tc += this._rows[i].tabCount;
        return tc;
    },
        ...
        this.obj.attachHTMLString = function (str) {
            ...
            for (var i = 0; i < z.length; i++) {
                ...
                if (window.execScript) {
                    ...
                    if (url)var s = dhtmlxAjax.getSync(url[2]).xmlDoc.responseText;
                    if (s)window.execScript(s);
                } else window.eval(s);
            }
        };
