...
function dhtmlxHierarchy() {
	...
	this.forEachChildF = function(id, funct, that, funct2) {
		...
		for ( var i = 0; i < z.childs.length; i++) {
			if (!funct.apply((that || this), [ z.childs[i] ]))
				continue;
			if (z.childs[i].childs.length)
				this.forEachChildF(z.childs[i].id, funct, that, funct2);
			if (funct2)
	...
	this.forEachChild = function(id, funct, that) {
		...
		for ( var i = 0; i < z.childs.length; i++) {
			...
			if (z.childs[i].childs.length)
				this.forEachChild(z.childs[i].id, funct, that)
		}
	};
	...
	this.addAfter = function(id, parentId, afterId, fix) {
		...
		if (z.state == dhtmlXGridObject._emptyLineImg)
			this.change(parentId, "state", (parentId == 0 ? "minus" : "plus"));
		if (afterId) {
			for ( var i = ind; i < z.childs.length; i++)
				z.childs[i].index++;
			z.childs = z.childs.slice(0, ind).concat([ x ]).concat(
					z.childs.slice(ind, z.childs.length))
		} else
			z.childs.push(x);
		this.get[id] = x;
		return x
	...
	this.remove = function(id) {
		...
		z.childs = [];
		z.parent.childs = z.parent.childs.slice(0, z.index).concat(
				z.parent.childs.slice(z.index + 1));
...
dhtmlXGridObject.prototype.collapseKids = function(curRow) {
	...
	if (r.state != "minus")
		return;
	if (!this.callEvent("onOpenStart", [ curRow.idd, 1 ]))
		return;
	var start = curRow.rowIndex;
	if (start < 0)
		start = this.rowsCol._dhx_find(curRow) + 1;
	...
	if (this._srnd || this.pagingOn) {
		...
		for ( var i = 0; i < len; i++)
			this.rowsCol[start + i].parentNode.removeChild(this.rowsCol[start
					+ i]);
...
dhtmlXGridObject.prototype._massInsert = function(r, start, ind, skip) {
	...
	;
	this.rowsCol = dhtmlxArray(this.rowsCol.slice(0, ind).concat(anew).concat(
			this.rowsCol.slice(ind)));
	...
	if (r.childs.length) {
		...
		if (r._xml_await) {
			...
			if (this.callEvent("onDynXLS", [ r.id ]))
				this.loadXML(this.kidsXmlFile + ""
						+ (this.kidsXmlFile.indexOf("?") != -1 ? "&" : "?")
						+ "id=" + r.id)
		}
	}
	;
...
dhtmlXGridObject.prototype.sortTreeRows = function(col, type, order) {
	...
	if (type == 'date')
		funct = function(a, b) {
			return (Date.parse(new Date(a._sort || "01/01/1900")) - Date
					.parse(new Date(b._sort || "01/01/1900")))
					* pos
		};
				...
				.forEachChild(
						...
						function(x) {
							if (x.buff.tagName == "TR") {
								var cs = (this._cssSP ? (x.level % 2)
										: (x.index % 2)) ? this._cssUnEven
										: this._cssEven;
								this.rowsAr[x.id].className = (cs + (this._cssSU ? (" "
										+ cs + "_" + x.level)
										: ""))
										+ " "
										+ (this.rowsAr[x.id]._css || "")
										+ ((this.rowsAr[x.id].className
												.indexOf("rowselected") != -1) ? " rowselected"
												: "")
							}
						}, this)
};
...
function eXcell_tree(cell) {
	...
	this.edit = function() {
		...
		this.er.innerHTML = "<"
				+ this.cell.atag
				+ " class='dhx_combo_edit' type='text' style='height:"
				+ (this.cell.offsetHeight - 6)
				+ "px;width:100%;border:0px;margin:0px;padding:0px;padding-top:"
				+ (_isFF ? 1 : 2) + "px;overflow:hidden;'></" + this.cell.atag
				+ ">";
		...
		this.er.firstChild.style.width = Math.max(0, this.cell.offsetWidth
				- this.er.offsetLeft)
				+ "px";
...
eXcell_tree.prototype.setValueA = function(content) {
	...
	this.grid.callEvent("onCellChanged", [ this.cell.parentNode.idd,
			this.cell._cellIndex, content ])
};
eXcell_tree.prototype.setValue = function(valAr) {
	if (this.cell.parentNode.imgTag)
		return this.setLabel(valAr);
	if ((this.grid._tgc.imgURL == null)
			|| (this.grid._tgc.imgURL != this.grid.imgURL)) {
		var _tgc = {};
		_tgc.spacer = "<img src='" + this.grid.imgURL
				+ "blank.gif' align='absmiddle' class='space'>";
		_tgc.imst = "<img src='" + this.grid.imgURL;
		_tgc.imsti = "<img src='" + (this.grid.iconURL || this.grid.imgURL);
		_tgc.imact = "' align='absmiddle' onclick='this."
				+ (_isKHTML ? "" : "parentNode.")
				+ "parentNode.parentNode.parentNode.parentNode.grid.doExpand(this);event.cancelBubble=true;'>"
		...
		_tgc.start = "<div class='treegrid_cell' style='overflow:hidden;white-space : nowrap;height:"
				+ (_isIE ? 20 : 18) + "px;'>";
		_tgc.itemim = "' align='absmiddle' "
				+ (this.grid._img_height ? (" height=\""
						+ this.grid._img_height + "\"") : "")
				+ (this.grid._img_width ? (" width=\"" + this.grid._img_width + "\"")
						: "")
				+ " ><span "
				+ ((_isFF || _isOpera) ? "style='position:relative;top:2px;'"
						: "") + "id='nodeval'>";
		_tgc.close = "</span></div>";
		this.grid._tgc = _tgc
	}
...
dhtmlXGridObject.prototype._process_tree_xml = function(xml, top, pid) {
	...
	if (main) {
		if (!rows.length)
			this._h2.change(pid, "state", dhtmlXGridObject._emptyLineImg);
	...
	if (ind != -1 && ind !== this.undefined) {
		...
		for ( var i = 0; i < len; i++)
			if (this.rowsCol[i + ind])
				this.rowsCol[i + ind].parentNode.removeChild(this.rowsCol[i
						+ ind]);
		if (this._fake)
			for ( var i = 0; i < len; i++)
				this._fake.rowsCol[i + ind].parentNode
						.removeChild(this._fake.rowsCol[i + ind]);
...
dhtmlXGridObject.prototype.addRow = function(new_id, text, ind, parent_id, img,
		child) {
	if (!this._h2)
...
dhtmlXGridObject.prototype.addRowBefore = function(new_id, text, sibl_id, img,
		child) {
	...
	return this.addRow(new_id, text, ind, this._h2.get[sibl_id].parent.id, img,
			child)
};
dhtmlXGridObject.prototype.addRowAfter = function(new_id, text, sibl_id, img,
		child) {
	var sb = this.rowsAr[sibl_id];
	if (!sb)
		return;
...
dhtmlXGridObject.prototype._copyTreeGridRowContent = function(frRow,
	...
	for (i = 0; i < frRow.cells.length; i++) {
		if (i != z)
			...
			this.cells(to_row_id, i).setValueA(
					this.cells(from_row_id, i).getValue())
