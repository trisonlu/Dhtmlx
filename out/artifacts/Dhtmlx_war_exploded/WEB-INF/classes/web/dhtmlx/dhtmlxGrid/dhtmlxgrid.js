...
//#}
	...
	*/
    ...
    this.setTextSelected=function(flag){
       this.textSelected=flag;
    }
	/**
...
//#}
	...
	*/
	...
	this.removeSelectionByRowId=function(row_id){
		this.editStop()
...
dhtmlXGridObject.prototype={
	...
	*/
	isTreeGrid:function(){
		return (this.cellType && this.cellType._dhx_find("tree") != -1);
	},
...
//#}
	loadXML:function(url, afterCall){
		if(this.pagingDisplay){
			if(url.indexOf("pagingDisplay=")==-1)
			  url = url + getUrlSymbol(url) +"pagingDisplay=" + this.pagingDisplay;
		}else{
		   if(url.indexOf("pagingDisplay=")==-1)
			url = url + getUrlSymbol(url) + "&pagingDisplay=default";
		}
...
//#xml_data:06042008{	
	_process_xml:function(xml){
		...
		this._zte_currentPage=parseInt(top.getAttribute("currentPage")||this.currentPage);
		this.rowCount=parseInt(top.getAttribute("rowCount")||0);
		this.rowsBufferOutSize=parseInt(top.getAttribute("pageSize")||this.rowsBufferOutSize);
		//////////////////////
		...
		var firstRowNum=(this.currentPage-1)*this.rowsBufferOutSize+1;
		var endRowNum=this.currentPage*this.rowsBufferOutSize<=this.rowCount?this.currentPage*this.rowsBufferOutSize:this.rowCount;
...
//#}	
	render_dataset:function(min, max){ 
		...
		//edit by lu.shuqin 分页后台每次查新数据，数组长度固定从0开始 start//
		if ( min>=max){
		  min=0;
		  max=this.rowsBufferOutSize;
		}
		//edit by lu.shuqin 分页后台每次查新数据，数组长度固定从0开始 end//
...
*/
	...
	*/
	getStateOfView:function(){
		if (this.pagingOn){
			var start = (this._zte_currentPage-1)*this.rowsBufferOutSize;
			return [this._zte_currentPage, start, Math.min(start+this.rowsBufferOutSize,this.rowCount), this.rowCount ];
		}
 		return [
